...
For Cloud Run, the Docker build optionally downloads and checksum-verifies both pinned artifacts, exports and warms both ONNX graphs, and stores them in the image. Runtime loads those read-only image files before readiness, avoiding Hugging Face downloads and graph export on each scale-from-zero start. Local Compose leaves `BAKE_MODELS=false` and uses its named model and ONNX-cache volumes with the MinIO preparation service. See [the Cloud Run build](../../deployment/cloudrun/cloudbuild.yaml) and [deployment guide](../DEPLOYMENT.md).
